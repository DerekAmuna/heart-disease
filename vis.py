...
def create_tooltip(country_code, metric, gender, selected_year=None):
    ...
    df = data[data['Code'] == country_code][['Year', col, 'Entity', 'obesity%', 't_htn_30-79', 't_high_bp_30-79', 't_htn_ctrl_30-79', 'gdp_pc', 'm_death_rate']].copy()
    if df.empty:
        return go.Figure(), {}
    ...
    risk_factors = {
        ...
        'Obesity Rate': format_value(year_data['obesity%'], is_estimate=is_estimate),
        ...
        'Hypertension Control': format_value(year_data['t_htn_ctrl_30-79'], is_estimate=is_estimate),
    ...
    fig.update_layout(
        ...
        title=dict(
            text=f"{df['Entity'].iloc[0]}<br>{metric}",
            x=0.5,
            xanchor='center',
            font=dict(size=10)
        ),
...
def create_chloropleth_map(filtered_data):
    ...
    """
    print("Creating chloropleth with data:", len(filtered_data) if filtered_data else "None")
    ...
    if not metric_col:
        print("No metric column found in:", df.columns)
        return go.Figure()
